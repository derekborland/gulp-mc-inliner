...
function apiAccess(key) {
	if(!key) throw new gutil.PluginError(PLUGIN, "MailChimp API Key required.");
...
function handleAPICall(strip) {
	var stream = through.obj(function(file, enc, cb) {
		...
		if(file.isStream()) {
			this.emit('error', new PluginError(PLUGIN, "Steams are not supported."));
			return cb();
		}

		api.call('helper', 'inline-css', {'html': file.contents.toString('utf8'), 'strip_css': strip}, function(err, data){
			if(err) {
				this.emit('error', new gutil.PluginError(PLUGIN, err.message));
			}	else {
